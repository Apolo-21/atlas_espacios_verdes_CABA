...
##############################################################################
# Estimar la inseguridad del entorno pedaleable de la Ciudad de Buenos Aires #
##############################################################################
...
# El siguiente script busca estimar la inseguridad del entorno pedaleable a 10 minutos
# desde el centroide de cada radio censal de la Ciudad de Buenos Aires (CABA). Por 
# este motivo, la estructura del mismo replica lo realizado en el script anterior,
# aunque adaptando los parámetros al uso de la bicicleta en el contexto urbano.....
...
################################################################################
# PARTICULARMENTE A ESTE SCRIPT, PODRIAMOS DARLE MÄS PESO A LOS SINIESTROS VIALES
################################################################################


# Vamos a transformar nuestro dataset de delitos en uno espacial. Para ello, primero
# corregimos aquellas coordenas que se enecuentran mal registradas.
delito <- delito %>% 
...
## Inseguridad del entorno pedaleable: Radios censales -------------------------
...
# Para comenzar, unimos espacialmente las iscronas base (de 10 minutos a bicicleta
# desde el centroide de cada radio censal) con los delitos, a fin de observar la 
# incidencia de estos últimos sobre el entorno caminable.
...
ponderador_hurto <- 0.10
ponderador_lesiones <- 0.50 # Peligrosidad de accidentes.
ponderador_robo <- 0.40
...
radios_caba_crimen <- radios_caba_crimen %>% 
    mutate(crime_index = hurto_index*ponderador_hurto + lesiones_index*ponderador_lesiones + robo_index*ponderador_robo,
           crime_pond= 1 - crime_index) %>% # ESTE PASO NO ESTÄ EN EL ISOCRONAS A PIE. LO SUMAMOS???
...
# Guardamos.
st_write(radios_caba_crimen, "data/processed/GCABA/crime/radios_con_indice_crime_CABA_BICI.shp",  delete_layer = TRUE)
