...
get_isocronas <- function(sf_object, minutos=10, pond_col="crime_pond", resolucion = 50, id_col = "id") {
    ...
    isocrona_con_id <- function(id, origen, pond_col) {
        print(paste("procesando ID", id, "- Ponderador:", pond_col))
 
        # Intentamos obtener isocrona

        resultado <- tryCatch(osrmIsochrone(origen, 
                                            breaks = c(0, (minutos*pond_col)), 
                                            res = resolucion, 
                                            returnclass = "sf"),
                              error = function(e) {
...
isocronas <- get_isocronas(radios_CABA_prueba, minutos = 10, resolucion = 50, pond_col=radios_CABA_prueba$crime_pond) %>% 
    select(id)
